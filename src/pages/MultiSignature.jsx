...
const MultiSignature = () => {
  ...
  const fetchSigners = async () => {
    try {
      const response = await fetch(`${API_BASE}/api/multisig/signers`);
  ...
  const fetchTransactions = async () => {
    try {
      const response = await fetch(`${API_BASE}/api/multisig/transactions`);
      const data = await response.json();
      if (response.ok) {
  ...
  const addSigner = async () => {
    ...
    setError("");
    try {
      const response = await fetch(`${API_BASE}/api/multisig/add-signer`, {
        method: "POST",
      ...
      setError(err.message || "Có lỗi xảy ra khi thêm người ký");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateTransaction = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/multisig/create-transaction`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const signTransaction = async (txId) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/multisig/sign-transaction/${txId}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({}),
      ...
      setError(err.message || "Có lỗi xảy ra khi ký giao dịch");
    } finally {
      setLoading(false);
    }
  };
  ...
  const executeTransaction = async (txId) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/multisig/execute-transaction/${txId}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({}),
      ...
      const data = await response.json();
      if (!response.ok)
        throw new Error(data.message || "Lỗi thực thi giao dịch");

      setSuccessMsg("Thực thi giao dịch thành công");
      await fetchTransactions();
      setTimeout(() => setSuccessMsg(""), 3000);
    } catch (err) {
      setError(err.message || "Có lỗi xảy ra khi thực thi giao dịch");
    } finally {
      setLoading(false);
    }
  };
                ...
                <div className="multisig-section signers-section">
                  <div className="section-header">
                    ...
                    <p className="section-subtitle">
                      Yêu cầu: {requiredSignatures}/{signers.length}
                    </p>
                  </div>
                        ...
                        <div key={idx} className="signer-item">
                          ...
                          <div className="signer-info">
                            <div className="signer-address">
                              {signer.substring(0, 10)}...
                              {signer.substring(signer.length - 8)}
                            </div>
                  ...
                  <div className="transactions-list">
                    {transactions.length > 0 ? (
                      transactions.map((tx) => (
                        <div
                          key={tx.id}
                          className={`transaction-item ${getStatusColor(
                            tx.status
                          )}`}
                          onClick={() =>
                            setSelectedTxId(
                        ...
                        >
                          ...
                          <div className="tx-info">
                            ...
                            <small className="tx-sigs">
                              Ký: {tx.signatures?.length || 0}/
                              {requiredSignatures}
                            </small>
                          </div>
                                  ...
                                  <ul className="signatures-list">
                                    {tx.signatures.map((sig, idx) => (
                                      ...
                                      >
                                        ...
                                        />
                                        <span>
                                          {sig.signer.substring(0, 10)}...
                                          {sig.signer.substring(
                                            sig.signer.length - 8
                                          )}
                                        </span>
                                        <small>
                ...
                >
                  ...
                  <div className="modal-content">
                    ...
                    <div className="info-box">
                      ...
                      <ul>
                        <li>
                          Giao dịch sẽ cần {requiredSignatures} chữ ký để được
                          phê duyệt
                        </li>
                        <li>
                          Hiện có {signers.length} người ký trong hệ thống
                        </li>
                        <li>
                          Sau khi được phê duyệt, giao dịch có thể được thực thi
                        </li>
                      </ul>
                    </div>
                  </div>
                  ...
                  <div className="modal-footer">
                    ...
                    <button
                      ...
                      disabled={
                        loading || !createFormData.description.trim()
                      }
                    >
