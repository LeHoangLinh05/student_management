...
// 🔥 Base URL cho API: lấy từ env, fallback localhost khi dev
...
const ZeroKnowledgeProof = () => {
  ...
  const generateProof = async () => {
    ...
    setError("");
    try {
      const response = await fetch(`${API_BASE}/api/zkp/generate-proof`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          proofType: zkpForm.proofType,
          ageThreshold: parseInt(zkpForm.ageThreshold, 10),
      ...
      setError(err.message || "Có lỗi xảy ra khi tạo proof");
    } finally {
      setLoading(false);
    }
  };
  ...
  const verifyProof = async () => {
    ...
    setError("");
    try {
      const response = await fetch(`${API_BASE}/api/zkp/verify-proof`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      setError(err.message || "Có lỗi xảy ra khi xác thực proof");
    } finally {
      setLoading(false);
    }
  };
                ...
                <div className="zkp-results-section">
                  ...
                  <div className="result-item">
                    ...
                    <div
                      className={`status-badge ${
                        zkpResults.verified ? "verified" : "unverified"
                      }`}
                    >
                      {zkpResults.verified
                        ? "✓ Đã xác thực"
                  ...
                  <div className="result-item">
                    ...
                    <div className="proof-value">
                      {zkpResults.proof.substring(0, 50)}...
                    </div>
                  </div>
                  ...
                  <div className="result-item">
                    ...
                    <pre>
                      {JSON.stringify(zkpResults.publicInputs, null, 2)}
                    </pre>
                  </div>
